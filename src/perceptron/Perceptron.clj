(ns perceptron.Perceptron
  (:gen-class
   :extends toolkit.SupervisedLearner
   :state state
   :init init
   :constructors {[java.util.Random] []}))
...
;; Redefine this to be useful later.
(defn -toString
  [this]
  "Hello, World!")
...
(defn perceptron-rule
  [weight lr result target inval]
...
(defn perceptron-recall
  [perceptron invec]
  (if (> (apply + (map * perceptron invec)) 0) 1 0))

(defn perceptron-train-datum
  [perceptron invec target]
...
(defn perceptron-train-datum-test
  [initialperceptron invec target]
